...
def get_us_quotes():
    ...
    if not tickers:
        return jsonify({})
    # Batch fetch via Yahoo Finance quote API (faster, one request)
    result = {}
    try:
        symbols = ','.join(tickers)
        r = requests.get(
            f'https://query1.finance.yahoo.com/v7/finance/quote?symbols={symbols}&fields=regularMarketPrice,regularMarketPreviousClose',
        ...
        if r.ok:
            ...
            for q in quotes:
                ...
                if price:
                    result[sym] = {'price': round(float(price),2), 'prev': round(float(prev),2) if prev else round(float(price),2)}
    except: pass
    # Fallback: try query2
    if not result:
        try:
            r2 = requests.get(
                f'https://query2.finance.yahoo.com/v7/finance/quote?symbols={symbols}&fields=regularMarketPrice,regularMarketPreviousClose',
                headers={'User-Agent':'Mozilla/5.0'},timeout=10)
            if r2.ok:
