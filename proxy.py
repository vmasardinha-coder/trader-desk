...
def get_us_quotes():
    ...
    # Method 1: TradingView america scanner (mais confiavel)
    tv_tickers = []
    for t in tickers:
        exc = _US_EXCHANGE.get(t, 'NASDAQ')
        tv_tickers.append(f'{exc}:{t}')
    try:
        r_tv = requests.post('https://scanner.tradingview.com/america/scan',
            json={'symbols':{'tickers':tv_tickers},'columns':['close','change_abs']},
            headers={'User-Agent':'Mozilla/5.0'},timeout=8)
        if r_tv.ok:
            for item in r_tv.json().get('data',[]):
                sym = item.get('s','').split(':')[-1]
                d = item.get('d',[])
                if d and d[0]:
                    close = round(float(d[0]),2)
                    chg = float(d[1]) if len(d)>1 and d[1] else 0
                    result[sym] = {'price':close,'prev':round(close-chg,2)}
    except: pass

    # Method 2: Yahoo Finance batch para os que faltaram
    ...
    if missing:
        try:
            ...
            if r_yh.ok:
                for q in r_yh.json().get('quoteResponse',{}).get('result',[]):
                    sym = q.get('symbol','')
