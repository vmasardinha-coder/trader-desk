...
| **🔴 PRIORIDADE — checagem retroativa de rompimento de barreira (KDO/KUO) em `em_analise`** | 🔴 **ABERTO, achado real do Victor em 05/08/2026, tratar como prioridade.** Causa raiz: `/analises/ranking` só roda Monte Carlo PROSPECTIVO (a partir do preço de HOJE pra frente) pra recalcular probabilidade — nunca compara o caminho de preço REAL desde `data_foto` contra `kdo`/`kuo`. Resultado: uma análise pode ter rompido a barreira de verdade há dias e o app continua mostrando probabilidade de sucesso alta, porque simplesmente não olha pra trás. Caso real pego: MUTC34 (`an_1784737588`, retorno controlado, KDO R$657,60) fechou em R$632,92 no 5º dia útil (29/07/2026) — rompeu, e ninguém foi avisado. Análise encerrada manualmente em 05/08/2026 (`motivo_encerramento: barreira_rompida_historico_real`). **Auditoria de segurança feita na mesma data**: as outras 7 análises `em_analise` com barreira (AMZO34, ROXO34, AXIA3, PRIO3, DIRR3, PETR4, SPCX34 ×2) foram checadas uma a uma contra o histórico real — todas OK, dentro do range. **Ação proposta**: adicionar rotina (rodar junto do `/analises/ranking` ou em job separado) que busca o histórico Yahoo desde `data_foto` de cada análise `em_analise` com barreira e sinaliza/fecha automaticamente se `min(close) <= kdo` ou `max(close) >= kuo`, em vez de depender de conferência visual manual no gráfico. |
