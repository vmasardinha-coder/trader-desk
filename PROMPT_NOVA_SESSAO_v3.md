...
| **Venda de Put a Seco** (Cash-secured Put) | Vender put sem ter a ação, recebendo prêmio, aceitando ficar comprado se exercido | 🔴 **Slot reservado, nada implementado** — `put_seco` já existe na lista de tipos válidos de ranking desde sessão anterior, mas nenhuma posição real usa isso, e não tem cálculo de probabilidade/prêmio dedicado. Se Victor trouxer uma operação dessas, vai cair no fluxo genérico sem modelagem própria. |
...
0. **[ACHADO HOJE, prioridade máxima] "Risco de overshoot" no retorno controlado** — pedido do Victor: hoje o sistema só mostra a probabilidade de TOCAR a barreira (risco de perda). Falta a métrica inversa: probabilidade de o papel fechar TÃO acima do previsto que o retorno fixo combinado (ex: 2,5%) deixa muito dinheiro na mesa vs. não ter feito nada. Ex: "qual a chance de eu ficar travado nos 2,5% enquanto o papel sobe 5%?" -- essa pergunta prática aparece toda vez que ele avalia uma estrutura nova, ainda não tem resposta no sistema. Adicionar como novo campo no ranking/foto: `prob_overshoot_pct` = P(variação final > teto_retorno), e talvez `overshoot_medio_pct` = quanto em média ele deixaria na mesa nesses casos.
